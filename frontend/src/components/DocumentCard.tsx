...
export default function DocumentCard({ document, onSelect, onOrganize, onDelete }: Props) {
  ...
  // The card used to be a <button> with more <button role="button" span>s nested
  // inside it — invalid HTML and unreliable keyboard behaviour. Now the card is
  // a plain container: one full-bleed <button> underneath handles "open", and
  // the corner actions are real sibling <button>s stacked above it.
  return (
    <div className="group relative flex aspect-[2/3] w-40 min-w-[160px] flex-col overflow-hidden rounded-lg bg-booki-card text-left shadow-lg transition-all duration-300 hover:-translate-y-2 hover:shadow-glow sm:w-48 sm:min-w-[192px]">
      <button
        type="button"
      ...
      <div className="pointer-events-none relative flex h-full flex-col items-center justify-center p-4 text-center">
      ...
      <div className="pointer-events-none absolute bottom-0 w-full p-3">
