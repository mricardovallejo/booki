...
- The core unit is the **session**: an open-ended reading journey with its own context and a recorded reading-progress marker. AI activities run on a separate, reader-controlled page range.
...
6. Set an activity page range (it starts as the pages read so far and follows
   along) that bounds the quiz, summaries and quick-actions, or ask BooKI to
   explain a passage or build a mnemonic in the same conversation.
