...
- **AiProfilesPage** — "Reading setup" (`/ai-profiles`, `/ai-profiles/:id`): two tabs, **Tutor profile** and **Reader profile**, same shape — a selector + `New` / `Duplicate` / `Delete`, the editor, then a `Save changes` button at the bottom.
  ...
  - *Reader profile* tab: `Name` / `Starting level` / shared `context`. It lands on one of the user's own profiles when any exist. Shipped templates such as "General reader" and "Dyslexia-friendly reader" are read-only and show an explanatory callout; `New` or `Duplicate` makes an editable one.
