...
## ADR-015: AI Profiles replace Profile Masters + user context (in progress)
...
- **Rollout**: Stage 1 — frontend + Node mock (built). Stage 2 — freeze the contract in `docs/openapi.yaml`. Stage 3 — backend: `ai_profiles` + `ai_profile_slot_prompts` tables, templates in a `SlotPromptCatalog` class (not the DB), a `PromptAssembler` replacing `SessionContextBuilder`, quiz/summary/capabilities reading their SlotPrompts, the data migration (seed user copies; `profile_masters` customizations → `persona`; `user.bio`/`systemPrompt` → the default profile's `reader_context`), remove `/api/profile-masters` and the old Java. Mechanics: `docs/prompts.md`.

- **Consequence**: `GET /sessions/{id}/context` returns every layer BooKI reads, grouped, not just the editable three. Deferred: a global "advanced prompts" screen (the per-profile editor covers it); switching a running session's profile (create a new session).
