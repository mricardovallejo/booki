...
- **Rollout**: Stage 1 — frontend + Node mock (built). Stage 2 — freeze the contract in `docs/openapi.yaml`. Stage 3 — backend: `ai_profiles` + `ai_profile_prompts` tables, a `PromptAssembler` replacing `SessionContextBuilder`, quiz/summary/capabilities reading their slots, the data migration (`profile_masters` → hidden templates + seeded user copies; `user.bio`/`systemPrompt` → the default profile's `reader_context` slot), remove `/api/profile-masters` and the old Java.
